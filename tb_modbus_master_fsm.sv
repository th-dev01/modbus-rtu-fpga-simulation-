...
module tb_modbus_master_fsm;
    ...
    task automatic send_partial_response(
        input int payload_length,
        input logic [7:0] payload [0:253],
        input int delay_cycles
    );
    ...
    task automatic expect_status(input modbus_status_t expected);
        begin
            wait (response_valid);
    ...
    initial begin
        ...
        response_register_read_index = 0;
        captured_count = 0;
        ...
        #1;
        assert (response_register_read_valid && response_register_read_data == 16'h1234);
        ...
        #1;
        assert (response_register_read_valid && response_register_read_data == 16'h5678);
        response_register_read_index = 8'd2;
        #1;
        assert (!response_register_read_valid && response_register_read_data == 16'h0000);
        ...
        expect_status(STATUS_EXCEPTION);
        assert (response_exception == 8'h02);

        // Resposta de excecao com codigo pouco comum: Gateway Target Device Failed to Respond.
        issue_command(8'h03, 16'hFFFF, 16'h0000, 16'h0001);
        payload[0]=8'h01; payload[1]=8'h83; payload[2]=8'h0B;
        send_response(3, payload, 1'b0, 1);
        expect_status(STATUS_EXCEPTION);
        assert (response_exception == 8'h0B);
        ...
        expect_status(STATUS_CRC_ERROR);
        ...
        expect_status(STATUS_OVERFLOW);
        assert (overflow_byte_count == 3);
        ...
        @(negedge clk);
        cmd_valid = 1'b0;
        expect_status(STATUS_UNSUPPORTED);
        ...
        cmd_register_addr = 16'h0000;
        cmd_write_data = 16'h0000;
        cmd_quantity = 16'h0000;
        cmd_valid = 1'b1;
        @(negedge clk);
        cmd_valid = 1'b0;
        expect_status(STATUS_INVALID_PARAM);
        assert (captured_count == 0);

        // Comando FC03 com quantidade acima do limite do protocolo Modbus.
        wait (cmd_ready);
        @(negedge clk);
        cmd_slave_addr = 8'h01;
        cmd_function = 8'h03;
        ...
        // Timeout depois de recepcao parcial.
        issue_command(8'h03, 16'h0000, 16'h0000, 16'h0001);
        payload[0]=8'h01; payload[1]=8'h03; payload[2]=8'h02;
        send_partial_response(3, payload, 1);
        expect_status(STATUS_TIMEOUT);
